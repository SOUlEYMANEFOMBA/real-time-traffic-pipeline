...
class KafkaDataSudOuestPublisherTask:
    """
    Cette classe publie des données formatées dans un topic Kafka - topic sud ouest.

    Attributes:
        None
    """
    
    def streaming_data_sud_ouest(self, doawloaddata, formatData):
        """
        Cette méthode envoie des données téléchargées et formatées au topic Kafka 'users_sud_ouest_created'.
        ...
        try:
            ...
            # Envoyer les données formatées au topic Kafka 'users_sud_ouest_created'
            producer.send('users_sud_ouest_created', json.dumps(format_data).encode('utf-8'))
